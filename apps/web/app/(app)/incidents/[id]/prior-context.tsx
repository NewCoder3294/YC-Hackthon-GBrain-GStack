    ...
    <div className="mt-8 space-y-3 border-t border-neutral-200 pt-6">
      <div className="flex items-baseline justify-between gap-2">
        <h2 className="font-mono text-[10px] uppercase tracking-widest text-neutral-500">
          Prior Context <span className="text-neutral-300">· GBrain</span>
        </h2>
      ...
      {queryErr ? (
        <p className="font-mono text-[10px] uppercase tracking-widest text-neutral-300">
          GBrain query failed: <span className="text-neutral-500">{queryErr}</span>
        </p>
      ) : hits.length === 0 ? (
        <p className="font-mono text-[10px] uppercase tracking-widest text-neutral-300">
          No prior records surfaced for this signal.
        </p>
      ) : (
        <ul className="space-y-2">
          {hits.map((h) => {
            ...
            return (
              <li
                key={h.id}
                className="border border-neutral-200 p-2 hover:border-black"
              >
                <div className="flex items-baseline justify-between gap-2">
      ...
      <p className="pt-1 font-mono text-[9px] uppercase tracking-widest text-neutral-300">
        retrieved via gbrain_prior_context · supabase RPC
      </p>
    </div>
  );
}
